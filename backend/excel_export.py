...
def build_excel(result: dict, nurses: list[Nurse], year: int, month: int, output: BytesIO):
    ...
    # 타이틀
    STAT_COLS = 10  # D, E, N, 6D, 6N, EDU, O, 근무, 시간, 요청
    ws.merge_cells(f"A1:{get_column_letter(3+num_days+STAT_COLS)}1")
    tc = ws["A1"]
    tc.value     = f"  {year}년 {month}월 간호사 근무표   ★=나이트전담  2=2교대  파=주2일제  ^=희망반영"
    ...
    stat_hdrs = ["D","E","N","6D","6N","EDU","O","근무","시간","요청"]
    ...
    for ri, n in enumerate(nurses):
        ...
        for d in days:
            ...
            c.font = _font(bold=is_req, size=9, color=SHIFT_TEXT_COLOR.get(sh, "000000"))
        ...
        for val, off, col in [
            ...
            (f"{fulfilled}/{total_req}", 10, "000000"),
        ]:
            c = ws.cell(row, 3+num_days+off, val)
            c.font=_font(bold=(off==8),size=9,color=col); c.fill=_fill(CLR["sum_bg"])
    ...
    hdrs2 = ["그룹","이름","속성","D","E","N","6D","6N","EDU","O","총근무","총시간","목표시간","요청"]
